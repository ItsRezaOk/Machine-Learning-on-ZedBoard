...
void runInferenceTest(const Model& model, const Path& basePath) {
    if (!Config::ENABLE_BENCHMARK_RUNS) {
        logInfo("--- Running Inference Test (single accelerated run) ---");
    ...
    logInfo("--- Running Inference Test (20-run average, NAIVE) ---");

    LayerData img(model[0].getInputParams(), basePath / "image_0.bin");
    img.loadData();
    logInfo("Input data loaded.");

    // accumulate per-layer and full inference times over N runs
    const int N = 20;
    std::vector<float> layerSums(model.getNumLayers(), 0.0f);
    float fullSum = 0.0f;
    ...
    for (int iter = 0; iter < N; ++iter) {
        ...
        fullSum += timer.milliseconds;

        // Note: Model::inference already emits per-layer timers when ENABLE_LAYER_TIMING is true.
        // We can't directly read those timers here, so we run a second pass to time each layer individually.
        for (std::size_t i = 0; i < model.getNumLayers(); ++i) {
            Timer t("L" + std::to_string(i));
            t.start();
            model.inferenceLayer((i == 0) ? img : model[i - 1].getOutputData(), static_cast<int>(i), Layer::InfType::NAIVE);
            t.stop();
            layerSums[i] += t.milliseconds;
        }

        // compare output only on first run to avoid extra load overhead each time
        if (iter == 0) {
            LayerData expected(model.getOutputLayer().getOutputParams(), basePath / "image_0_data" / "layer_12_output.bin");
            expected.loadData();
            output.compareWithinPrint<fp32>(expected);
    ...
    // Print averages
    logInfo("--- Averages over " + std::to_string(N) + " runs ---");
    for (std::size_t i = 0; i < model.getNumLayers(); ++i) {
        float avg = layerSums[i] / N;
        logInfo("Avg L" + std::to_string(i) + ": " + std::to_string(avg) + "ms");
    }
    logInfo("Avg Full Inference: " + std::to_string(fullSum / N) + "ms");
}
// -------------------------------------------------------------------------
