...
class Layer {
   public:
    ...
    // Layer Type
    enum class LayerType { NONE, DENSE, SOFTMAX, PATCHEMBED, TRANSFORMERBLOCK, TOKENEXTRACT, LAYERNORM };
...
inline ui32 float_to_u32(float val) {
    ui32 result;
    memcpy(&result, &val, sizeof(ui32));
    return result;
}

// Helper to cast u32 bits back to float
inline float u32_to_float(ui32 val) {
    float result;
