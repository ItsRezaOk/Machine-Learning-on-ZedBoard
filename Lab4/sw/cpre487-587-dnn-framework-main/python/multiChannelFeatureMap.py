...
file_path1 = 'img_data/layer_outputs/layer_output.bin'
file_path2 = 'img_data/layer_outputs/layer_0_output.bin'
...
if __name__ == "__main__":
    ...
    try:
        ...
        is_4d = shape_len == 4
        if shape_len == 4:  # B, H, W, C
            map1_slice = np.mean(data1[batch_index_to_plot, :, :, :], axis=-1)
            map2_slice = np.mean(data2[batch_index_to_plot, :, :, :], axis=-1)
            print(f"Computing average across channels for 4D shape: [Batch {batch_index_to_plot}, :, :, :]")
        
        elif shape_len == 3:  # H, W, C
            map1_slice = np.mean(data1[:, :, :], axis=-1)
            map2_slice = np.mean(data2[:, :, :], axis=-1)
            print(f"Computing average across channels for 3D shape: [:, :, :]")
