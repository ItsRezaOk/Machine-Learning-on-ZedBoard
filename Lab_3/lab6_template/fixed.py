...
# Plotting function generated by Gemini 2.5 pro for debugging
...
output_to_bin("layer_11_output", "layer_outputs", all_activations[len(all_activations)-1])

# For the final layer, optionally quantize and compare if needed
final_activation = all_activations[-1][0]  # Assuming batch size 1000, take first
final_quant = quantize_inputs(final_activation, activation_scales[-1], zero_points[-1])
final_dequant = (final_quant.astype(np.float32) - zero_points[-1]) / activation_scales[-1]
final_original_flat = final_activation.flatten()
final_mae = np.mean(np.abs(final_original_flat - final_dequant.flatten()))
final_mse = np.mean((final_original_flat - final_dequant.flatten())**2)
final_max_err = np.max(np.abs(final_original_flat - final_dequant.flatten()))
print(f"Final Layer - MAE: {final_mae}, MSE: {final_mse}, Max Error: {final_max_err}")
