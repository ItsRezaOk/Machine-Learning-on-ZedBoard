...
#error "Unsupported QUANT_BITS (must be 2, 4, or 8)"
    ...
    {
#ifdef ZEDBOARD
        //std::cout << "Using Zedboard MLP Accelerator for Convolutional Layer " << getLayerNum() << "\n";
        // --- Quantization Parameters ---
        const float scales[] = INPUT_SCALES;
        const float weight_scales[] = WEIGHT_SCALES;
        const int8_t zero_points[] = ZERO_POINTS;
        ...
        const float scale_weight = weight_scales[layerNumber];
        const int8_t zp_next = zero_points[layerNumber + 1];
        const bool swap_state = getDoMaxPool();
        ...
        // DEBUG output all the scales
        // for (int i = 0; i < 8; i++){
        //     scale_input = scales[layerNumber + i];
        //     scale_next = scales[layerNumber + 1 + i];
        //     scale_weight = weight_scales[layerNumber + i];
        //     long double mult = static_cast<long double>(scale_next) /
        //             (static_cast<long double>(scale_weight) * static_cast<long double>(scale_input));

        //     int32_t fixed_mult = static_cast<int32_t>(std::round(mult * (1 << 28)));
        //     int32_t q_scale_int = fixed_mult;
        //     std::cout << "Mult out layer "<< (int) i << ": " << (long double) mult << "\n";
        // }
        ...
        const int inputWidth = inputDims[0];
        // const int inputDepth = inputDims[2];
        const int outputHeight = getDoMaxPool() ? outputDims[1] * 2 : outputDims[1];
        const int outputWidth = getDoMaxPool() ? outputDims[0] * 2 : outputDims[0];
        const int outputDepth = outputDims[2];

        //ui8 *filter_banks[4] = {MLP_FILTER0, MLP_FILTER1, MLP_FILTER2, MLP_FILTER3};
        int8_t *currentFilterPointer = (int8_t*) getWeightData().raw();
        int filterSize = filterHeight * filterWidth * filterDepth;

        // Compute pre-pool convolution output dims
        ...
        int input_end_diff_fw = 1 - filterWidth + inputWidth;
        int input_end_diff_fh = input_end_diff_fw - (inputWidth * filterHeight) + (inputWidth  * inputHeight);
        int input_end_diff_fc = input_end_diff_fh - (inputWidth * inputHeight * filterDepth) + 1;
        int input_end_diff_ow = input_end_diff_fc + (filterWidth - 1);

        // Set fixed layer configurations (outside loop)
        Xil_Out32(MLP_FILTER_W, filterWidth);
        Xil_Out32(MLP_FILTER_H, filterHeight);
        Xil_Out32(MLP_FILTER_C, filterDepth);
        Xil_Out32(MLP_OUTPUT_W, conv_out_width);  // Pre-pool for index_gen loops
        Xil_Out32(MLP_OUTPUT_H, conv_out_height); // Pre-pool
        ...
        size_t elements_per_channel = getDoMaxPool() ? ((int)(outputWidth * outputHeight / 4)) : outputHeight * outputWidth;
        //std::cout << "Elements per channel: " << elements_per_channel << "\n";
        ...
        Xil_Out32(MLP_Q_ZERO, static_cast<int32_t>(zp_next));

        // Base control bits (RELU always on, max pooling if enabled)
        uint32_t ctrlb_base = MLP_CTRLB_RELU | (getDoMaxPool() ? MLP_CTRLB_MAX_POOLING : 0) | (swap_state ? MLP_CTRLB_SWAP_ACTIVATIONS : 0);
        //std::cout << "Do Max Pooling: " << getDoMaxPool() << "\n";
        ...
        // Number of groups
        const int num_groups = outputDepth / 4;

        //std::cout << "Finished setting constants for layer " << getLayerNum() << "\n";
        ...
        // memcheck(getOutputData().raw(), MLP_OUTPUTS, getOutputData().getParams().byte_size());
        // std::cout << "Output sizeL " << getOutputData().getParams().byte_size() << " bytes copied to output data.\n";
#else
