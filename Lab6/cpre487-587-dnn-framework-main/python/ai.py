...
def quantize_biases(biases, scale_bias, zero_point_sum):
    ...
    quantized = quantized.astype(np.int32)  # No clip; use full int32 range
    return quantized
...
for i, layer_name in enumerate([layer.name for layer in model.layers]):
    ...
    if 'conv' in layer_name or 'dense' in layer_name:
        ...
        if 'conv' in layer_name:
            quantized_weights = np.transpose(quantized_weights, (3, 2, 0, 1))
        elif 'dense' in layer_name:
            quantized_weights = np.transpose(quantized_weights, (1, 0))
    ...
    if 'pool' in layer_name or 'flatten' in layer_name:
        ...
        if i > 0:
            ...
            if 'pool' in layer_name:
                ...
                quantized_trans = np.transpose(quantized_input, (2, 0, 1))
                quantized_activation = quantized_trans.reshape(-1)
    ...
    quantized_to_save = quantized_activation
    if len(quantized_activation.shape) == 3:
        quantized_to_save = np.transpose(quantized_to_save, (2, 0, 1))
    flat_to_save = quantized_to_save.flatten()
    output_to_bin(f"layer_{i}_output", "layer_outputs", flat_to_save)
