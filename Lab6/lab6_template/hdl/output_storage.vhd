...
architecture Behavioral of output_storage is
    ...
    signal cnt_ox, cnt_oy, cnt_c, cnt_oxMAX, cnt_oyMAX : unsigned(DIM_WIDTH-1 downto 0) := (others => '0');
...
begin
    ...
    product2_max <= unsigned(resize(unsigned(cnt_oyMAX) * shift_right(unsigned(output_w), 1), 32));
    
    final_sum <= unsigned(initial_offset) + product1 + product2 + unsigned(cnt_ox);
    final_sum_max <= unsigned(initial_offset) + product1 + product2_max + unsigned(cnt_oxMAX);
    
    BRAM_clk <= clk;
    
    oddColumnTest <= is_odd_column;
    oddRowTest <= is_odd_row;
    ...
    process(S_AXIS_TVALID, convolutionComplete, currentState) is   
       begin
...
begin
    ...
    elsif rising_edge(clk) then -- Synchronous logic on Rising Edge
...
--                            BRAM_en <= '1';
            ...
            when WRITEBRAM =>
                ...
                if (max_pooling = '1' and (is_odd_row = '0' or is_odd_column = '0')) then
                    if(offset = 0) and signed(BRAM_dout(7 DOWNTO 0)) > value2 then
                        BRAM_din <= BRAM_dout;  
                    elsif (offset = 1) and signed(BRAM_dout(15 DOWNTO 8)) > value2 then
                        BRAM_din <= BRAM_dout;      
                    elsif (offset = 2) and signed(BRAM_dout(23 DOWNTO 16)) > value2 then
                        BRAM_din <= BRAM_dout; 
                    elsif (offset = 3) and signed(BRAM_dout(31 DOWNTO 24)) > value2 then
                        BRAM_din <= BRAM_dout;
                    elsif(offset = 0) then
                    ...
                    if(max_pooling = '0') then
                        ...
                        if cnt_c = 3 then 
                            ...
                            if(is_odd_column = '1') then 
                                ...
                                if cnt_oxMAX = shift_right(unsigned(output_w), 1) - 1 then
                                    cnt_oxMAX <= (others => '0');
                                    ...
                                    if(is_odd_row = '1') then 
                                        ...
                                        if cnt_oyMAX = shift_right(unsigned(output_h), 1) - 1 then
                                            cnt_oyMAX <= (others => '0');
                                        else
                                            cnt_oyMAX <= cnt_oyMAX + 1;
                                        end if;
                                    end if;
                                else
                                    cnt_oxMAX <= cnt_oxMAX + 1;
                                end if;
                            end if;
